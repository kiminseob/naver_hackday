...
#   방송/연예 기사 크롤링   # 
def broadcast(headers,title_num,end_num, page_num,count): #기사 제목,내용,날짜 크롤링
	...
	uploaded = uploadDate(headers,url_params,page_num)  #해당 페이지에 있는 20개의 기사 업로드 날짜 크롤링
	...
	for i in range(int(title_num),end_num,-1): #전체 기사 갯수만큼 for문을 반복
		...
		if count%21==0: #한 페이지에 기사가 20개씩 있으므로 20개마다 한 페이지 넘어감.
			count=1
			page_num+=1
			uploaded =uploadDate(headers,url_params,page_num)  #페이지 넘어갈 때마다 20개의 기사 업로드 날짜 크롤링
...
#   방송/연예 기사 올라온 날짜 크롤링   #
def uploadDate(headers, url_params, page_num):
