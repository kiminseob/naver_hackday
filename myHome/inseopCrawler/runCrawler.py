...
def start():
	...
	for i in range(8):
		if i==0:
			...
			proc = Process(target=newsCrawler.broadcast, args=(headers,title_num[0],divide,1,0))#방송/연예기사 크롤링
		else:
			if i==7:
				#print("i=7 : title_num:%d, divide:0, page:%d, count:%d"%(divide,next_page,next_count))
				proc = Process(target=newsCrawler.broadcast, args=(headers,divide,0,next_page,next_count))#방송/연예기사 크롤링
			else:
				title_num[0]=divide
				divide = int(title_num[0])-divided_index
				#print("else : title_num:%d , divide:%d, page:%d, count:%d"%(title_num[0],divide,next_page,next_count))
				proc = Process(target=newsCrawler.broadcast, args=(headers,title_num[0],divide,next_page,next_count))#방송/연예기사 크롤링
